...
describe('TestCryptoFreezer', () => {
    ...
    const asUser: AsWalletFunction = asWalletFactory(user)
    const asDeployer: AsWalletFunction = asWalletFactory(deployer)
    const asUserTwo: AsWalletFunction = asWalletFactory(userTwo)
        ...
        describe('Token supported', () => {
            ...
            it('Allows to deposit ETH using different address', async () => {
                ...
                await expect(asDeployer(freezer)['depositETH(uint256,uint256,address)'](
                    unlockTimeUTC, infinity(),
                    user.address, {value: value})).to.emit(freezer, 'NewDeposit').withArgs(
                    zeroAddress(), user.address, value, unlockTimeUTC, infinity(), 0,
                )
            ...
            it('Doesn\'t allow to deposit for longer period than max time lock period', async () => {
                ...
                await expect(asUser(freezer)['depositETH(uint256,uint256)'](
                    unlockTimeUTC, infinity(), {value: utils.parseEther('10')}))
                    .to.be.revertedWith('Time lock period too long')
            })
            ...
            it('Doesn\'t allow to deposit for unlock time set in the past', async () => {
                ...
                await expect(asUser(freezer)['depositETH(uint256,uint256)'](
                    unlockTimeUTC, infinity(), {value: value}))
                    .to.be.revertedWith('Unlock time set in the past')
            })
            ...
            it('Adds to ERC20 deposit', async () => {
                ...
                await expect(asUser(freezer)['depositERC20(address,uint256,uint256,uint256)'](
                    token.address, value,
                    unlockTimeUTC, infinity())).to.emit(freezer, 'NewDeposit').withArgs(
                    token.address, user.address, value, unlockTimeUTC, infinity(), 0,
                )

                await expect(asUser(freezer)['addToDepositERC20(uint256,uint256)'](0, value))
                    .to.emit(freezer, 'AddToDeposit').withArgs(
            ...
            it('Does not allow to add to deposit if requirements not satisfied (ERC20)', async () => {
                ...
                await expect(asUser(freezer)['addToDepositERC20(uint256,uint256)'](0, 0))
                    .to.be.revertedWith('Value is 0')

                await expect(asUser(freezer)['addToDepositERC20(uint256,uint256)'](1, value))
                    .to.be.revertedWith('Invalid deposit index')

                await expect(asUser(freezer)['depositETH(uint256,uint256)'](unlockTimeUTC, infinity(), {value: value}))
                    .to.emit(freezer, 'NewDeposit').withArgs(
                        zeroAddress(), user.address, value, unlockTimeUTC, infinity(), 1)

                await expect(asUser(freezer)['addToDepositERC20(uint256,uint256)'](1, value))
                    .to.be.revertedWith('Adding to wrong deposit type (ERC20)')
            })
            ...
            it('Does not allow to add to deposit if requirements not satisfied (ETH)', async () => {
                ...
                await expect(asUser(freezer)['addToDepositETH(uint256)'](0, {value: 0}))
                    .to.be.revertedWith('Value is 0')

                await expect(asUser(freezer)['addToDepositETH(uint256)'](1, {value: value}))
                    .to.be.revertedWith('Invalid deposit index')

                await expect(asUser(freezer)['depositERC20(address,uint256,uint256,uint256)'](
                    token.address, value, unlockTimeUTC, infinity())).to.emit(freezer, 'NewDeposit')
                    .withArgs(token.address, user.address, value, unlockTimeUTC, infinity(), 1)

                await expect(asUser(freezer)['addToDepositETH(uint256)'](1, {value: value}))
                    .to.be.revertedWith('Adding to wrong deposit type (ETH)')
            })
            ...
            describe('withdrawERC20', () => {
                ...
                describe('using sinon', () => {
                    ...
                    it('Cannot add to a deposit if unlocked', async () => {
                        await expect(asUser(freezer)['addToDepositERC20(uint256,uint256)'](0, value))
                            .to.be.revertedWith('Deposit is unlocked')
                    })
                })
            })
            ...
            describe('withdrawETH', () => {
                ...
                describe('using sinon', () => {
                    ...
                    it('Cannot withdraw using wrong function', async () => {
                        await expect(asUser(freezer)['withdrawERC20(address,uint256)'](user.address, 0))
                            .to.be.revertedWith('Withdrawing wrong deposit type (ERC20)')
                    })
                    ...
                    it('Cannot withdraw whe owner address is 0', async () => {
                        await expect(asUser(freezer)['withdrawETH(address,uint256)'](zeroAddress(), 0))
                            .to.be.revertedWith('Owner address is 0')
                    })
                    ...
                    it('Cannot add to an already withdrawn deposit', async () => {
                        ...
                        await expect(asUser(freezer)['addToDepositETH(uint256)'](0, {value: value}))
                            .to.be.revertedWith('Deposit does not exist')
                    })

                    it('Cannot add to a deposit if unlocked', async () => {
                        await expect(asUser(freezer)['addToDepositETH(uint256)'](0, {value: value}))
                            .revertedWith('Deposit is unlocked')
                    })
                })
            })
            ...
            describe('Withdraws ERC20 deposit, which has been added to first', () => {
                ...
                beforeEach(async () => {
                    ...
                    await expect(asUser(freezer)['addToDepositERC20(uint256,uint256)'](0, value))
                        .to.emit(freezer, 'AddToDeposit').withArgs(user.address, 0, value)
                })
            ...
            describe('Withdraws ETH deposit, which has been added to first', async () => {
                ...
                beforeEach(async () => {
                    await expect(asUser(freezer)['depositETH(uint256,uint256)'](
                        ...
                        .withArgs(zeroAddress(), user.address, value, unlockTimeUTC, infinity(), 0)
    ...
    describe('withdraw with price fetcher set', () => {
        ...
        describe('simple withdraw ERC20', () => {
            beforeEach(async () => {
                ...
                await expect(asUser(freezer)['depositERC20(address,uint256,uint256,uint256)'](
                    token.address, value, unlockTimeUTC, minPrice))
                    .to.emit(freezer, 'NewDeposit').withArgs(
            ...
            it('Withdraws when price is above min limit from different account', async () => {
                ...
                await expect(asUserTwo(freezer)['withdrawERC20(address,uint256)'](user.address, 0))
                    .to.emit(freezer, 'Withdraw')
                    .withArgs(token.address, user.address, value, 0, unlockTimeUTC, minPrice)
        ...
        describe('simple withdraw ETH', () => {
            beforeEach(async () => {
                ...
                await expect(asUser(freezer)['depositETH(uint256,uint256)'](
                    unlockTimeUTC, minPrice, {value: value}))
                    .to.emit(freezer, 'NewDeposit').withArgs(
            ...
            it('Withdraws when price is above min limit from different account', async () => {
                ...
                await expect(asUserTwo(freezer)['withdrawETH(address,uint256)'](user.address, 0))
                    .to.emit(freezer, 'Withdraw')
                    .withArgs(zeroAddress(), user.address, value, 0, unlockTimeUTC, minPrice)
    ...
    describe('migration', () => {
        ...
        beforeEach(async () => {
            ...
            migrationAgent = await deployContract(
                deployer, MigrationAgentMock, [freezer.address, targetFreezer.address])
